...
```
※ SSHキーをご利用の方は、GitHubの「<> Code」ボタンよりSSH URLを取得してクローンしてください。
...
プロジェクトのフォルダに移動したら、以下のコマンドを**1回実行するだけ**で、開発用・テスト用の環境がすべて整います！
...
* **コンテナのビルド・起動**とパッケージのインストール
* **設定ファイル**（`.env` / `.env.testing`）の作成とアプリケーションキーの生成
* **開発用・テスト用データベース**の構築と権限付与
* データベースの**マイグレーション**と**ダミーデータの投入**（シーディング）
* **必須ディレクトリ**（`storage`, `bootstrap/cache`）への適切な権限設定

※上記の手動設定は一切不要です。コマンド完了後、すぐに動作確認や自動テストを始められます！
...
## 🚀 動作確認・テスト
アプリケーションが要件を満たし、正常に動作することを「自動テスト」と「手動」の両面から確認済みです。
...
### ✅ 手動による動作確認
以下の手順で実際のユーザーフローに沿って動作確認を行っていただけます。
